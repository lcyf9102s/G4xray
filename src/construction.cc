...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    G4Box *solidRadiator = new G4Box("solidRadiator", 0.5*m, 0.5*m, 0.1*m); // solid radiator
    G4LogicalVolume *logicRadiator = new G4LogicalVolume(solidRadiator, H2O, "logicRadiator"); // logical  radiator
    ...
    // 探测器的logical空间
    G4Box *solidDetector = new G4Box("solidDetector", 0.05*m, 0.05*m, 0.01*m); // solid detector
